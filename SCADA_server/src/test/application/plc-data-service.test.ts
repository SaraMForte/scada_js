...
import PlcDataService, {
    PlcDatasResoult,
} from '../../main/application/plc-data-service'
import {
    PropertyError,
...
describe('Readvalue', () => {
    ...
    test('GIVEN properties with values WHEN read an non-existing property THEN throws error', async () => {
        const plcDataService = new PlcDataService(
            dummyPlcData1Repository,
            new DummyPlcDataRepository({}),
        )

        await expect(plcDataService.readValue('Z')).rejects.toThrow(
            PropertyNotFoundError,
        )
    })
    ...
    test('GIVEN properties with values WHEN read a property duplicated across multiple Plc THEN throws error', async () => {
        ...
        await expect(plcDataService.readValue('A')).rejects.toThrow(
            RepeatedPropertyError,
        )
    })
})
...
describe('ReadValues', () => {
    ...
    test('GIVEN repositories with data WHEN reading specific data THEN return data with no errors', async () => {
        ...
        expect(
            await plcDataService.readValues(Object.keys(DUMMY_DATA_OBJ_1)),
        ).toEqual(dummyValues1)
        expect(
            await plcDataService.readValues(Object.keys(DUMMY_DATA_OBJ_2)),
        ).toEqual(dummyValues2)
    })
    ...
    test('GIVEN repositories with error to read WHEN attempting to read PLCs THEN returns no data with errors', async () => {
        ...
        expectReadValues.error.forEach((err) =>
...
describe('writeValue', () => {
    ...
    test('GIVEN repositories with data WHEN writing non-existing property THEN throws error', async () => {
        ...
        await expect(plcDataService.writeValue('Z', 40)).rejects.toThrow(
            PropertyNotFoundError,
        )
    })
    ...
    test('GIVEN repositories with data WHEN fail to write properties THEN throws error', async () => {
        const plcDataService = new PlcDataService(
            new DummyErrorWriteRepository(),
        )

        await expect(plcDataService.writeValue('A', 60)).rejects.toThrow(
            PropertyWriteError,
        )
    })
})
...
describe('writeValues', () => {
    ...
    test('GIVEN repositories with mapped data WHEN writing existing properties THEN properties are updated correctly', async () => {
        ...
        const dummyValues1Map = new Map(
            Object.entries(dummyPlcData1Repository.dummyData),
        )
        const dummyValues2Map = new Map(
            Object.entries(dummyPlcData2Repository.dummyData),
        )
    ...
    test('GIVEN repositories with mapped data WHEN writing exisiting properties THEN properties return errors', async () => {
        ...
        writeResults.forEach((error) =>
            expect(error).toEqual(expect.any(PropertyError)),
        )
    })
    ...
    test('GIVEN repository WHEN fail attempting to write values THEN return errors', async () => {
        ...
        result.forEach((err) =>
            expect(err).toEqual(expect.any(PropertyWriteError)),
        )
    })

    // test('GIVEN repository WHEN ', async () => {
    //     const dummyDataMap = new Map([['A', 13]])
    //     const mockErrorRead = 'strange error test'

    //     const mockRepository = sinon.createStubInstance(ModbusPlcDataRepository)
    //     const plcDataService = new PlcDataService(mockRepository)

    //     const findPropertyRepositoryStub = sinon.stub((plcDataService as any)['#findPropertyRepository'])

    //     mockRepository.writeValues.resolves()
